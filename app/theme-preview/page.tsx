    ...
    <div className="flex flex-col h-screen bg-gray-50 overflow-hidden">
      ...
      <header className="flex-none bg-white border-b border-gray-200 px-3 sm:px-4 md:px-6 py-3 sm:py-4 flex items-center justify-between z-30 shadow-sm">
        <div className="flex items-center gap-2 sm:gap-4 min-w-0">
          ...
          <button
            onClick={handleBack}
            className="p-2 hover:bg-gray-100 rounded-full transition-colors text-slate-500 shrink-0"
          >
        ...
        <div className="flex bg-gray-100 rounded-lg p-0.5 sm:p-1 gap-0.5 sm:gap-1 shrink-0">
          <button
            onClick={() => setViewMode("desktop")}
            disabled={isMobileDevice}
            title={
              isMobileDevice
                ? "Desktop view not available on mobile"
          ...
          >
            <Monitor size={14} className="sm:w-4 sm:h-4 shrink-0" />
          ...
          >
            <Smartphone size={14} className="sm:w-4 sm:h-4 shrink-0" />
      ...
      <div className="flex flex-1 overflow-hidden relative">
                ...
                >
                  <div className="relative z-10">
                    <div className="flex justify-between items-start">
                      ...
                      {activeTheme.id === theme.id && (
                        <div className="text-blue-500 bg-white rounded-full p-0.5 shadow-sm shrink-0">
                  ...
                  <div className="flex gap-1.5 sm:gap-2 mt-3 sm:mt-4 relative z-10">
                    <div
                      className="w-5 h-5 sm:w-6 sm:h-6 rounded-full border border-gray-200 shadow-sm"
          ...
          <div className="flex-none bg-slate-900 text-white border-t border-slate-800 shadow-[0_-10px_40px_rgba(0,0,0,0.2)] z-20 transition-all duration-300">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {!isPromptOpen && (
                  <span className="hidden lg:inline text-xs text-slate-500 ml-2 font-normal normal-case opacity-0 group-hover:opacity-100 transition-opacity">
                    Click to expand
                  </span>
                )}
              </div>
              {isPromptOpen ? (
                <ChevronDown
                  size={16}
                  className="sm:w-[18px] sm:h-[18px] text-slate-400 group-hover:text-white"
                />
              ) : (
                <ChevronUp
                  size={16}
                ...
                <div className="group relative bg-slate-800 rounded-lg p-2.5 sm:p-3 border border-slate-700 hover:border-slate-500 transition-colors">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleCopy(activeTheme.promptShort, "short");
                      }}
                ...
                <div className="group relative bg-slate-800 rounded-lg p-2.5 sm:p-3 border border-slate-700 hover:border-slate-500 transition-colors">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleCopy(activeTheme.promptLong, "long");
                      }}
                      className="text-slate-500 hover:text-white transition-colors"
        ...
        <main className="flex-1 bg-gray-100/50 p-4 sm:p-6 lg:p-8 flex items-center justify-center overflow-auto relative">
          ...
          <div
            className={`transition-all duration-500 ease-in-out z-10 w-full flex items-center justify-center ${
              viewMode === "mobile"
                ? "scale-100"
                : "max-w-5xl h-[70vh] sm:h-[75vh] lg:h-[80vh]"
            }`}
          >
