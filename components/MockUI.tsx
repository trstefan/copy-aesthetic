...
import { BarChart2, Bell, Home, Mail, Settings, User, Search, Plus } from 'lucide-react';
...
const MockUI: React.FC<MockUIProps> = ({ theme, isMobile }) => {
  ...
  const buttonStyle: React.CSSProperties = {
    ...
    color: theme.id === 'minimalist' || theme.id === 'neobrutalism' ? '#fff' : 'var(--bg-primary)', // Contrast hack
  ...
  if (isMobile) {
    return (
      <div 
        className="w-[320px] h-[640px] overflow-hidden relative flex flex-col shadow-2xl mx-auto border-[12px] border-gray-900 rounded-[3rem]"
      ...
      >
        ...
        <div className="px-5 pt-8 pb-3 flex justify-between items-center z-10">
          <div className="flex flex-col">
            <span className="text-[10px] font-semibold uppercase opacity-50 tracking-wide" style={{color: 'var(--text-secondary)'}}>Welcome back</span>
            <span className="text-lg font-bold mt-0.5">Alex Doe</span>
          </div>
          <div className="w-9 h-9 rounded-full flex items-center justify-center" style={cardStyle}>
        ...
        <div className="flex-1 px-5 space-y-3 overflow-y-auto no-scrollbar z-10 pb-20">
            ...
            <div className="p-4" style={cardStyle}>
                <div className="flex justify-between items-start mb-1.5">
                    <span className="text-xs" style={{color: 'var(--text-secondary)'}}>Total Balance</span>
                    <span className="text-green-500 text-[10px] font-mono">+2.4%</span>
                </div>
                <div className="text-2xl font-bold mb-3">$12,450</div>
                <div className="h-1.5 w-full bg-gray-200/20 rounded-full overflow-hidden">
                    <div className="h-full w-2/3" style={{backgroundColor: 'var(--accent)'}}></div>
                </div>
            </div>
                ...
                {['Send', 'Request', 'More'].map((action, i) => (
                    <button 
                        ...
                        style={{
                            ...
                            color: i === 0 ? (theme.id === 'minimalist' ? '#fff' : 'var(--bg-primary)') : 'var(--text-primary)',
            ...
            <div className="space-y-2.5">
                 <h3 className="text-[11px] font-bold uppercase tracking-wide opacity-60 mt-1">Recent Activity</h3>
                 {[1, 2, 3].map((i) => (
                     <div key={i} className="p-2.5 flex items-center gap-2.5" style={cardStyle}>
                         <div className="w-8 h-8 rounded-full flex items-center justify-center bg-gray-500/10 flex-shrink-0">
                             <User size={14} />
                         </div>
                         <div className="flex-1 min-w-0">
                             <div className="text-xs font-bold truncate">Design Subscription</div>
        ...
        {/* Mobile Nav */}
        <div className="absolute bottom-0 left-0 right-0 h-14 flex items-center justify-around z-20 px-4" style={{
            backgroundColor: 'var(--bg-secondary)', 
            ...
            <div className="w-11 h-11 -mt-6 rounded-full flex items-center justify-center shadow-lg" style={{ backgroundColor: 'var(--accent)', color: theme.id === 'minimalist' ? '#fff' : 'var(--bg-primary)' }}>
                <Plus size={22} />
            </div>
  ...
  return (
    ...
    >
      {/* Sidebar */}
      <div className="w-64 flex flex-col p-6 border-r" style={{borderColor: 'var(--border)', backgroundColor: isGradient ? 'rgba(0,0,0,0.1)' : 'var(--bg-secondary)'}}>
        <div className="flex items-center gap-2 mb-10">
            <div className="w-8 h-8 rounded" style={{backgroundColor: 'var(--accent)'}}></div>
            <span className="font-bold text-lg">Acme Corp</span>
        </div>
        <nav className="flex-1 space-y-2">
            {['Dashboard', 'Analytics', 'Team', 'Projects', 'Documents'].map((item, i) => (
                <div 
                    key={item} 
                    className={`p-3 rounded cursor-pointer flex items-center gap-3 ${i === 0 ? 'opacity-100' : 'opacity-60 hover:opacity-100'}`}
                    style={{
                        backgroundColor: i === 0 ? 'var(--bg-primary)' : 'transparent',
                        fontWeight: i === 0 ? 'bold' : 'normal',
                        boxShadow: i === 0 ? 'var(--box-shadow)' : 'none',
                    }}
                >
                    <div className="w-2 h-2 rounded-full" style={{backgroundColor: i === 0 ? 'var(--accent)' : 'transparent'}}></div>
                    {item}
                </div>
            ))}
        </nav>
        <div className="p-4 rounded mt-auto" style={cardStyle}>
            ...
            <div className="text-sm font-bold mb-2">85% Used</div>
             <div className="h-1 w-full bg-gray-200/20 rounded-full overflow-hidden">
                <div className="h-full w-4/5" style={{backgroundColor: 'var(--accent)'}}></div>
            </div>
        </div>
      </div>
      ...
      <div className="flex-1 p-8 overflow-y-auto">
        <header className="flex justify-between items-center mb-10">
            ...
            <div className="flex gap-4">
                <button className="px-4 py-2 rounded text-sm font-medium" style={cardStyle}>Filter</button>
                <button className="px-4 py-2 rounded text-sm font-medium shadow-lg" style={buttonStyle}>New Project</button>
            </div>
        </header>
        ...
        <div className="grid grid-cols-3 gap-6 mb-8">
            {[
                ...
                <div key={i} className="p-6 flex flex-col gap-4" style={cardStyle}>
                    ...
                    <div className="text-xs text-green-500 font-mono">{stat.change} vs last month</div>
                </div>
            ))}
        </div>

        {/* Large Chart Area Placeholder */}
        <div className="w-full h-64 rounded p-6 flex items-center justify-center relative overflow-hidden" style={cardStyle}>
